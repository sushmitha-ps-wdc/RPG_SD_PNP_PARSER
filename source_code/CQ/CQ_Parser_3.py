...
class CQParser:
    ...
    def __init__(self,excel_file_name, log_directory_path):
        ...
        self.list_CA_substring = []
        self.specified_subscripts_nos = {"CA01":17, "CA02":15, "CA03":10, "CA04":5, "CA05":16,
                                         "CA06":20, "CA07":16,"CA08":5,"CA09":15,"CA10":8,
                                         "CA11":18,"CA12":25,"CA13":12,"CA14":15, "CA15":10,
                                         "CA16":10,"CA17":5,"CA18":5,"CA19":13,"CA20":14,
                                         "CA21":7,"CA22":6,"CA23":10,"CA24":19,"CA25":11,"CA26":11,
                                         "CA27":5, "CA28":15, "CA29":10 }
    ...
    def excel_styling(self):
        ...
        def apply_alignment(sheet):
            for row in sheet.iter_rows(min_row=1, max_row=sheet.max_row, min_col=1, max_col=sheet.max_column + 1):
                        ...
                        for row, status in zip(self.start_index_list_non_ca, self.status_list_non_ca):
                            if status == "FAIL":
                                for failure in self.error_line_list:
                                    ...
                                    sheet.cell(row=row, column=8, value=failure)
                                    sheet.cell(row=row, column=7).font = self.red_font
                                    sheet.cell(row=row, column=8).font = self.red_font
                            elif status == "STOPPED":
                                sheet.cell(row=row, column=7, value=status)
                                sheet.cell(row=row, column=7).font = self.red_font
                            else:
    ...
    def merging_cells_of_all_scripts(self):
        ...
        try:
            for start, end in zip(self.start_index_list, self.len_tc_name_list):
                ...
                sheet[f"F{start}"].value = sheet[f"F{end_cell + 1}"].value
            ...
            for row_idx, spec_val, actual_val in zip(modified_start_list, self.specified_subscripts_nos,
                                                     self.len_tc_name_list):
            ...
            for start, end in zip (modified_start_list, modified_end_list):
                ...
                sheet.merge_cells(f"F{start}:F{end}")
            ...
            try:
                with open(txt_file_full_path, 'r') as file:
                    ...
                    if txt_file not in file_path_column:
                        ...
                        if row_index is not None:
                            self.start_index_list.append(row_index)
                            for testcase_name, status in zip((reversed(self.testcase_name_list)), reversed(self.sub_script_status_list)):
                                sheet.insert_rows(row_index, amount=1)
                                sheet.cell(row=row_index, column=5, value=testcase_name)
                                ...
                                if status == "FAIL":
                                    print(self.error_line_list)
                                    for failure in self.error_line_list:
                                        sheet.cell(row=row_index, column=8, value=failure)
                                        sheet.cell(row=row_index, column=8).font = self.red_font
                                if status == "STOPPED":
                                    sheet.cell(row=row_index, column=7).font = self.red_font
    ...
    def creating_allscript_name(self):
        try:
            ...
            for log_name in self.log_files_list:
                current_col = openpyxl.utils.get_column_letter(tc_column_num)
                log_name_value = log_name.split(".")[0]
                if log_name.startswith('CA'):
                    list__to_map_spec_value = log_name[0:4]
                    self.list_CA_substring.append(list__to_map_spec_value)
                    self.ca_log_names.append(log_name_value)
                sheet[f"{current_col}{current_row}"].value = log_name_value
                current_row += 1
            wb.save(self.excel_file_name)
            print(self.list_CA_substring)
            self.specified_subscripts_nos = [self.specified_subscripts_nos[item] for item in self.list_CA_substring if
                                             item in self.specified_subscripts_nos]
            print(self.specified_subscripts_nos)
            print("Successfully update the log file names as ALLScripts Name ")

            for log_name in self.log_files_list:
                full_path = os.path.join(self.log_directory_path, log_name)
                with open(full_path, 'r') as file:
                    ...
                    for line in lines:
                        if '--randomseed' in line:
                            val = line.split(':')[1]
                            value = int(val.strip())
                            self.random_seed_list.append(value)
            print(self.random_seed_list)
            current_row = 2
            rs_column_num = 6
            ...
            for random_seed in self.random_seed_list:
                current_col = openpyxl.utils.get_column_letter(rs_column_num)
                sheet[f"{current_col}{current_row}"].value = random_seed
                current_row += 1

            wb.save(self.excel_file_name)
            print("Successfully update the Random seed number for each  ALLScripts Name ")
    ...
    def creating_CQ_Parser_excel_template(self):
        try:
            ...
            sheet['F1'] = "Random Seed"
            sheet['F1'].font = Font(bold = True)

            sheet['G1'] = "Status"
            sheet['G1'].font = Font(bold=True)
            sheet['H1'] = "Failure Details"
