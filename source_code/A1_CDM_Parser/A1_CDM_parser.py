...
class A1CDMParser:
    ...
    def __init__(self, excel_file_name, log_directory_path):
        ...
        self.search_string = ["Random Read 4KiB (Q=  1,T= 1) :", "Random Write 4KiB (Q=  1,T= 1) :"]
    ...
    def extracting_values_from_log_files(self):
        ...
        for txt_file, column_name in zip(self.log_files_list, self.column_name_list):
            ...
            with open(txt_file_full_path, 'r', encoding='utf-8', errors='ignore') as file:
                ...
                for string in self.search_string:
                    if string in txt_content:
                        ...
                        for line in lines:
                            if string in line:
                                value = (str(line.split("[")[1]).strip().split(" ")[0])
            ...
            try:
                ...
                print("Successfully updated the extracted values to A1 CDM File")
            except Exception as err:
    ...
    def iterating_over_log_directory(self):
        try:
            ...
            self.log_files_list = [file for file in self.log_files_list if file.endswith('.txt')]
    ...
    def creating_A1_CDM_excel_template(self):
        try:
            workbook = openpyxl.Workbook()
            sheet = workbook.active
            sheet['A1'] = "Application Performance (A1) Using CDM and Lexar Card Reader"
            sheet['A2'] = "Operation"
            ...
            sheet['B4'] = float("500")
            workbook.save(self.full_excel_file_path)
            print("Successfully created the A1_CDM Excel Template")
        except Exception as err:
            print(f"Error observed while creating the {self.excel_file_name} file")

def input_from_cmd_line():
    print("=" * 55 + "A1 CDM Parsing Started" + "=" * 55)
    file_name = input("Enter the A1 CDM Parsing file name: ")
...
if __name__ == "__main__":
    ...
    A1_CDM = A1CDMParser(excel_file_name, log_directory_path)
    A1_CDM.creating_A1_CDM_excel_template()
    A1_CDM.iterating_over_log_directory()
    A1_CDM.creating_sample_column()
    A1_CDM.extracting_values_from_log_files()
    A1_CDM.excel_styling()