...
class A1TMParser:
        ...
        for html_file, column_name in zip(self.log_files_list, self.column_name_list):
            ...
            with open(html_file_full_path, 'r') as file:
                ...
                try:
                    ...
                    print("Successfully updated the extracted values to A1 TM File")
                except Exception as err:
    ...
    def creating_A1_TM_excel_template(self):
        try:
            workbook = openpyxl.Workbook()
            sheet = workbook.active
            sheet['A1'] = "A1 Performance using TM "
            ...
            workbook.save(self.full_excel_file_path)
            print("Successfully created the A1 TM Excel Template")
        except Exception as err:
            print(f"Error observed while creating the {self.excel_file_name} file")

def input_from_cmd_line():
    print("=" * 55 + "A1 TM Parsing Started" + "=" * 55)
    file_name = input("Enter the A1 TM Parsing file name: ")
...
if __name__ == "__main__":
    ...
    print(f"Entered A1 TM file name is {excel_file_name}")
    print(f"Entered log files directory is {log_directory_path}")
    A1_TM = A1TMParser(excel_file_name, log_directory_path)
    A1_TM.creating_A1_TM_excel_template()
    A1_TM.iterating_over_log_directory()
    A1_TM.creating_sample_column()
    A1_TM.extracting_values_from_log_files()
    A1_TM.excel_styling()