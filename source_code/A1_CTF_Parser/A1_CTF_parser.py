...
class A1CTFParser:
    ...
    def extracting_values_from_log_files(self):
        ...
        for txt_file, column_name in zip(self.log_files_list, self.column_name_list):
            ...
            try:
                ...
                print("Successfully updated the extracted values to A1 CTF File")
            except Exception as err:
    ...
    def iterating_over_log_directory(self):
        try:
            self.log_files_list = os.listdir(log_directory_path)
    ...
    def creating_A1_CTF_excel_template(self):
        try:
            workbook = openpyxl.Workbook()
            sheet = workbook.active
            sheet['A1'] = "A1 Performance using CTF "
            ...
            sheet['B4'] = float("500")
            workbook.save(self.full_excel_file_path)
            print("Successfully created the A1 CTF Excel Template")
        except Exception as err:
            print(f"Error observed while creating the {self.excel_file_name} file")

def input_from_cmd_line():
    print("=" * 55 + "A1 CTF Parsing Started" + "=" * 55)
    file_name = input("Enter the A1 CTF Parsing file name: ")
...
if __name__ == "__main__":
    ...
    print(f"Entered A1 CTF file name is {excel_file_name}")
    print(f"Entered log files directory is {log_directory_path}")
    A1_CTF = A1CTFParser(excel_file_name, log_directory_path)
    A1_CTF.creating_A1_CTF_excel_template()
    A1_CTF.iterating_over_log_directory()
    A1_CTF.creating_sample_column()
    A1_CTF.extracting_values_from_log_files()
    A1_CTF.excel_styling()